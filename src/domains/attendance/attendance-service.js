...
class AttendanceService {
  ...
  async checkIn(currentUser, file = [], data) {
    ...
    // Phase 1: validate + create row di transaksi singkat (no S3 upload).
    // Tujuan: hindari nahan koneksi DB selagi upload S3 ~3-8s saat peak load.
    const attendance = await this.prisma.$transaction(async (tx) => {
      ...
      // Pre-check duplicate (cepat return friendly message kalau memang sudah)
      ...
      // Geofence: kalau di luar radius -> TOLAK
      ...
      // Create attendance dengan checkInPhoto null dulu — diisi setelah S3 upload sukses.
      // Wrap try/catch P2002 untuk handle race antar 2 request concurrent yang
      // lolos pre-check duplicate (unique constraint yang nge-save di DB level).
      let createdAttendance;
      try {
        createdAttendance = await tx.attendance.create({
          data: {
            date,
            checkInTime: now,
        ...
        if (err?.code === "P2002") {
          // Race: request lain udah create row barusan (window antara findUnique dan create)
          throw BaseError.badRequest("You have already checked in today");
        }
        throw err;
      }
    ...
    // Phase 2: upload S3 OUTSIDE transaksi. Lock DB udah lepas.
    // Kalau upload gagal, row attendance tetap exist tanpa foto — bisa di-retry
    // atau di-update manual via admin. Lebih aman daripada rollback yang bikin
    // user gak bisa check-in karena unique constraint.
    let photoUrl = null;
    try {
      photoUrl = await this.s3Service.uploadFile(file[0], "checkInPhotos");
    } catch (uploadErr) {
      console.error(
        "[checkIn] S3 upload failed for attendance",
        attendance.id,
        uploadErr,
      );
      // Return attendance tanpa photo — anggap sukses agar user gak retry & bikin "already checked in"
      return attendance;
    }

    // Phase 3: patch attendance dengan photo URL
